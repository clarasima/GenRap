...
const source = computed(() => {
  return (props.item?.match) ? props.item.chosen : ((props.item?.dblp) ? 'dblp' : ((props.item?.scholar) ? 'scholar' : 'interface'));
});
...
<template>
  <div class="publication-item">
    <div v-if="props.editIndex === props.index" style="width:100%">
      <EditPublication :item="props.item" :index="props.index" @save="saveEdit" @cancel="cancelEdit" type="Edit" />
    </div>
    <div v-else style="width:100%">
      <div class="checkbox-container" v-if="fromReport && props.item?.edited">
